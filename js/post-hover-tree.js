...
var watcherRunned = false;
var x=0;
var y=0;

function showPostPreview(e, num, thread)
{
    ...
    var clon = $('#reply_'+num).clone();
    clon.addClass('post_hover');
    clon.addClass('post_hover' + num);

    x += (e.target.offsetLeft);
    y += (e.target.offsetTop - $('#reply_'+num).height());

    clon.css('left', x +'px');
    clon.css('top', y +'px');

    $(clon).on('mouseover', '.post-link', function(e)
    {
        var $el = $(this);
        var num2 = $el.data('id');
        var thread = 0;//$el.data('thread');
     
        showPostPreview(e, num2, thread);
    
    });

    $(clon).hide();
    $(clon).appendTo('.hover-container');
    $(clon).fadeIn(300);
}   

function Watcher()
{
    if($('.post_hover:hover').length == 0 && $('.post-link:hover').length == 0)
    {
        x=0, y=0;
...
$('#threads').on('mouseover', '.post-link', function(e)
{
    var $el = $(this);
    var num = $el.data('id');
    var thread = 0;//$el.data('thread');
